...
import com.target.project.activity.TargetActivity;
...
public class UmengAnalysisRequest extends FreshRequest<UmengAnalysisResponse> {
	...
	public String getUrl() {
		return ApiURL.URL_UMENG_ANALYSIS;
	}
	...
	public Map<String, String> getHeaders() {
		String header = "Android/"+Constant.UMNEG_SDK_VERSION+" $#_appname%2F$#_appversion+sdk%2F$#_osversion+$#_idmd5";
		try {
			header = header.replace("$#_appname", URLEncoder.encode(context
					.getText(context.getApplicationInfo().labelRes).toString(),
					"utf-8"));
		} catch (Exception e) {
			e.printStackTrace();
		}

		header = header.replace("$#_appversion",
				MiscUtils.getAppVersion(context));
		header = header.replace("$#_osversion", Build.VERSION.CODENAME);
		header = header.replace("$#_idmd5",
				MD5.encrypt(MiscUtils.getIMEI(context)));
		super.getHeaders().put("X-Umeng-Sdk", header);
//		super.getHeaders().put("Content-Type", "text/plain");
