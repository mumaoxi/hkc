...
#存放各个id的键值
id_item_map = {'anim':'0x7f03ffff','array':'0x7f0affff','attr':'0x7f00ffff','bool':'0x7f08ffff','color':'0x7f05ffff','dimen':'0x7f09ffff','drawable':'0x7f01ffff','id':'0x7f06ffff','layout':'0x7f02ffff','raw':'0x7f04ffff','string':'0x7f06ffff','style':'0x7f07ffff'}
	...
	try:
		...
		for item in items:
			for key in id_item_map.keys():
				...
				if cmp(key,v_type) == 0 and cmp(v_id,id_item_map[key]) > 0 and v_name.find("hkc_") == -1:
...
def move_res_file_to_target():
	...
	#移动hkc，资源文件
	os.system("mkdir -v "+target_file_path+"/smali/com/hkc")
	os.system("cp -R "+extra_file_path+"/smali/com/hkc/* "+target_file_path+"/smali/com/hkc/")
	...
	os.system("cp -R "+extra_file_path+"/smali/"+target_package_name.replace(".","/")+"/* "+target_file_path+"/smali/"+target_package_name.replace(".","/")+"/")
...
def merge_public_xml():
	try:
		...
		print "\n#### move hkc_res xml..."
		assets_res_parse = ET.parse(target_file_path+"/assets/hkc_res.xml");
