	...
	for parent,dirnames,filenames in os.walk(file_path):    #三个参数：分别返回1.父目录 2.所有文件夹名字（不含路径） 3.所有文件名字
		...
		for filename in filenames:                        #输出文件信息
			...
			if cmp(filename , "AndroidManifest.xml")==0:
				...
				replaceStr(path,"http://schemas.android.com/apk/res/"+package_orig,"http://schemas.android.com/apk/res/"+package_new);
				# print "" + path
...
def mvFiles(file_path,package_orig,package_new):
	...
	os.system("mv "+file_path+"/smali/"+package_orig.replace(".","/")+"/R* "+file_path+"/smali/"+package_new.replace(".","/")+"/");

#反编译包
def reverseApk(apk_path):
	os.system("java -jar apktool.jar d "+apk_path)
...
package_orig = apk_info.package_name
